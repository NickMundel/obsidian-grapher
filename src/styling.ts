  ...
  return function stylingPlugin(instance: chartType) {
    if (!instance.listenerCount("after:draw")) {
      instance.on("after:draw", () => {
        ...
        selection.selectAll(".axis-label").style(
          "font-size",
          `${plugin.settings.labelFontSize}px`,
        ).style("fill", plugin.settings.fontColor);

        selection.selectAll(".origin").style(
          "stroke",
          plugin.settings.lineColor,
        ).style("stroke-width", `${plugin.settings.lineWidth}px`).style(
          "opacity",
          1,
        );

        selection.selectAll(".tick line").style(
          "stroke",
          plugin.settings.gridColor,
        ).style("stroke-width", `${plugin.settings.gridWidth}px`).style(
          "opacity",
          1,
        );

        selection.selectAll("line.text").style(
          "fill",
          plugin.settings.fontColor,
        ).style("font-size", `${plugin.settings.scaleFontSize}px`);

        selection.selectAll(".domain").style(
          "stroke",
          plugin.settings.gridColor,
        ).style("stroke-width", `${plugin.settings.gridWidth}px`).style(
          "opacity",
